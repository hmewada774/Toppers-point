  ...
  fetch('/api/toppers')
    ...
    .then(data => {
      ...
      function renderToppers() {
        ...
        data
          .filter(t => (year === 'all' || String(t.year) === String(year)) && (subject === 'all' || String(t.subject) === String(subject)))
          .forEach(t => {
            ...
            const cls = String(t.className || t.class || '');
            // lines
            const p1 = document.createElement('p');
            p1.textContent = `Class ${cls}`;
            const isSenior = cls === '11' || cls === '12';
            if (isSenior) {
              const pSub = document.createElement('p'); pSub.textContent = `Subject: ${t.subject || ''}`; card.appendChild(pSub);
              const pPerc = document.createElement('p'); pPerc.textContent = `Total Percentage: ${t.marks}%`; card.appendChild(pPerc);
            } else {
              const pPerc = document.createElement('p'); pPerc.textContent = `Percentage: ${t.marks}%`; card.appendChild(pPerc);
            }

            const small = document.createElement('small');
            small.textContent = String(t.year);
      ...
      if (yearSelect) yearSelect.onchange = renderToppers;
      if (subjectSelect) subjectSelect.onchange = renderToppers;
      renderToppers();
    });
}
  ...
  fetch('/header.html')
    ...
    .then(html => {
      ...
      document.querySelectorAll('header nav a').forEach(a => {
        ...
        if (route === current || (route === '/' && current === '/index.html')) {
      ...
      if (!navOverlay){ navOverlay = document.createElement('div'); navOverlay.className = 'nav-overlay'; document.body.appendChild(navOverlay); }
      ...
      if (menuToggle && mainNav) {
        ...
        document.addEventListener('click', (e) => {
          if (!headerFromFile.contains(e.target) && mainNav.classList.contains('active')) {
            menuToggle.classList.remove('active');
            mainNav.classList.remove('active');
            document.body.classList.remove('lock');
            navOverlay.classList.remove('show');
